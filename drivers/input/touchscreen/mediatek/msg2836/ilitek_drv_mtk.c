...
//    const char *vcc_i2c_name = "vcc_i2c";
    ...
    g_I2cClient = client;
    TPD_DMESG("g_I2cClient->addr = 0x%02x\n", g_I2cClient->addr); 
    ...
    TPD_DMESG("TPD probe done: tpd_load_status = %d\n",tpd_load_status);
...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip, "0x%02x", g_ChipType);
	sprintf(current_tp_info.id,"0x%02x",g_I2cClient->addr);
	sprintf(current_tp_info.vendor, "mstar");
	DrvGetCustomerFirmwareVersionByDbBus(EMEM_MAIN, &major, &minor, &p_version);
	sprintf(current_tp_info.more,"0x%x,ver:%s",major,(char *)p_version);
