...
s32 IicWriteData(u8 nSlaveId, u8* pBuf, u16 nSize)
{
    ...
    if (g_I2cClient != NULL)
    {
        ...
        else
        {
            ...
            //TPD_DMESG("---g_I2cClient->addr = 0x%02x---",g_I2cClient->addr);
#ifdef CONFIG_ENABLE_DMA_IIC
			//TPD_DMESG("---CONFIG_ENABLE_DMA_IIC---");
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
    TPD_DMESG("--- MS_TS_MSG_IC_GPIO_RST =%d ---",MS_TS_MSG_IC_GPIO_RST);
	TPD_DMESG("--- MS_TS_MSG_IC_GPIO_INT =%d ---",MS_TS_MSG_IC_GPIO_INT);
...
void DrvCheckFWSupportDriver(void)
{
    ...
    mdelay(I2C_SMBUS_READ_COMMAND_DELAY_FOR_PLATFORM);
    IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 4); 
    sscanf(DEVICE_DRIVER_RELEASE_VERSION,"%c.%c.%c.%c", &szDriverVer[0], &szDriverVer[1], &szDriverVer[2], &szDriverVer[3]);
    TPD_DMESG("Read FW support Driver Version: %d.%d.%d.%d\n", szRxData[0], szRxData[1], szRxData[2], szRxData[3]);
    ...
    (szRxData[0] << 24) + (szRxData[1] << 16) + (szRxData[2] << 8) + szRxData[3]){
        ...
        TPD_DMESG("Please upgreade your driver. The FW support driver %d.%d.%d.%d\n", szRxData[0], szRxData[1], szRxData[2], szRxData[3]);
    }
}
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
	...
	if(g_FwVersionFlag)
	{
		TPD_DMESG("*** ILITEK %s() Major = %d, Minor = %d.%d, _gFwVersion = %s ***\n", __func__, nMajor, (nMinor & 0xFF), ((nMinor >> 8) & 0xFF), _gFwVersion);
	}
	else
	{
		TPD_DMESG("*** ILITEK %s() Major = %d, Minor = %d, _gFwVersion = %s ***\n", __func__, nMajor, nMinor, _gFwVersion);
	}
#ifdef CONFIG_UPDATE_FIRMWARE_BY_SW_ID
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
