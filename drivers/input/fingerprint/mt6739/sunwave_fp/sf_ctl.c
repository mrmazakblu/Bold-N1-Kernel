...
//---------------------------------------------------------------------------------
#define SF_DRV_VERSION "v2.2.29-2018-08-20"
...
#ifdef CONFIG_PM
static int sf_pm_suspend(struct device *device)
{				/* wake up */
	xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __FUNCTION__);
	return 0;
}

static int sf_pm_resume(struct device *device)
{				/* wake up */
	xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __FUNCTION__);
	return 0;
}

static const struct dev_pm_ops sf_pm_ops = {
	.suspend = sf_pm_suspend,
	.resume = sf_pm_resume,
};
#endif
