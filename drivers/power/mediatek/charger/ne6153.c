...
static int ne6153_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	PRINTK("%s\n",__func__);
	//info = devm_kzalloc(&client->dev, size`of(*info), GFP_KERNEL);
    //if (!info)
    //    return -ENOMEM;
#if 0
	info->pctl_wrx = devm_pinctrl_get(&client->dev);
	if (!IS_ERR(info->pctl_wrx)){
		pstate_default = pinctrl_lookup_state(info->pctl_wrx,"default");
		if (!IS_ERR(pstate_default)){
			pinctrl_select_state(info->pctl_wrx,pstate_default);
		}else{
			PRINTK("set wrx default state fail<%d>\n", PTR_ERR(pstate_default));
		}
		//info->pst_wrx_en_in = pinctrl_lookup_state(info->pctl_wrx,"wrx_en_in");
		//if (IS_ERR(info->pst_wrx_en_in)){
		//	PRINTK("get wrx wrx_en_in state fail<%d>\n", PTR_ERR(info->pst_wrx_en_in));
		//}
		//info->pst_wrx_en_l = pinctrl_lookup_state(info->pctl_wrx,"wrx_en_l");
		//if (IS_ERR(info->pst_wrx_en_l)){
		//	PRINTK("get wrx wrx_en_l state fail<%d>\n", PTR_ERR(info->pst_wrx_en_l));
		//}
		info->pst_sw_flag_h = pinctrl_lookup_state(info->pctl_wrx,"sw_flag_h");
		if (IS_ERR(info->pst_sw_flag_h)){
			PRINTK("get wrx sw_flag_h state fail<%d>\n", PTR_ERR(info->pst_sw_flag_h));
		}
		info->pst_sw_flag_l = pinctrl_lookup_state(info->pctl_wrx,"sw_flag_l");
		if (IS_ERR(info->pst_sw_flag_l)){
			PRINTK("get wrx sw_flag_l state fail<%d>\n", PTR_ERR(info->pst_sw_flag_l));
		}
	}else{
		PRINTK("get pinctrl fail<%d>\n",PTR_ERR(info->pctl_wrx));
		return -EINVAL;
	}
	...
	mutex_lock(&switch_lock);
	if (sw_sel_pending){
		if (!IS_ERR(info->pst_sw_flag_h)){	//usb
			wrx_disable_vout();
			pinctrl_select_state(info->pctl_wrx,info->pst_sw_flag_h);
		}
	}else{
