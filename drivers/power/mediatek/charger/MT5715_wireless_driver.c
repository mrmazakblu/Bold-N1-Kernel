...
//prize modify by sunshuai, wireless charge MT5715  soft It is used to increase the offset of the RX to the received power of the TX, for example, the actual reception is 5W, and the report is 5W+offset. 20190621-start
		val.ptr[0] = 0x34;
		val.ptr[1] = 0x03;
		MT5715_write_buffer(mte, 0x0074, val.ptr, 2);//Increase the received power increase reported by RX to TX when the current is 300ma-450ma is 0.6W.
		pr_err("%s:write 0x0074 addr  value: 0x%02x,0x%02x \n",__func__, val.ptr[0],val.ptr[1]);

		val.ptr[0] = 0x33;
		val.ptr[1] = 0x03;
		MT5715_write_buffer(mte, 0x0076, val.ptr, 2);//Increase the received power increase reported by RX to TX when the current is 450ma-600ma is 0.6W.
		pr_err("%s:write 0x0076 addr  value: 0x%02x,0x%02x \n",__func__, val.ptr[0],val.ptr[1]);


        val.ptr[0] = 0x31;
		val.ptr[1] = 0x05;
		MT5715_write_buffer(mte, 0x0078, val.ptr, 2);//Increase the received power increase reported by RX to TX when the current is 600ma-750ma is 0.32W.
		pr_err("%s:write 0x0078 addr  value: 0x%02x,0x%02x \n",__func__, val.ptr[0],val.ptr[1]);


        val.ptr[0] = 0x32;
		val.ptr[1] = 0x03;
		MT5715_write_buffer(mte, 0x007A, val.ptr, 2);//Increase the received power increase reported by RX to TX when the current is 750ma-1.2a is 0.7W.
		pr_err("%s:write 0x007A addr  value: 0x%02x,0x%02x \n",__func__, val.ptr[0],val.ptr[1]);

//prize modify by sunshuai, wireless charge MT5715	soft It is used to increase the offset of the RX to the received power of the TX, for example, the actual reception is 5W, and the report is 5W+offset. 20190621-end
	}
